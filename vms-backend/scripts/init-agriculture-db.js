...
async function initializeAgriculturDatabase() {
  let connection;
  try {
    ...
    await connection1.execute('CREATE DATABASE IF NOT EXISTS vms_agriculture');
    await connection1.end();
    console.log('✓ Agriculture Database exists or created');

    // Now connect to the vms_agriculture database
    connection = await mysql.createConnection(config);
    console.log('Connected to VMS Agriculture database');
    ...
    await connection.execute(`INSERT IGNORE INTO users (user_id, user_email, password_hash, user_mobile) VALUES 
      (1, 'admin@agriculture.gov.in', 'admin', '9876543210'),
      (2, 'hod@agriculture.gov.in', 'hod', '9876543211'),
      (3, 'clerk@agriculture.gov.in', 'clerk', '9876543212')
    `);
    console.log('✓ Test users seeded (admin@agriculture.gov.in, hod@agriculture.gov.in, clerk@agriculture.gov.in)');
    ...
    // Insert office
    await connection.execute(`INSERT IGNORE INTO offices (offices_id, office_name) VALUES 
      (1, 'Agriculture Dept. HQ')
    `);
    ...
    await connection.execute(`INSERT IGNORE INTO officers (officers_id, officer_name, officer_designation, user_id, officer_status_id) VALUES 
      (1, 'Priya Sharma', 'Chief Director', 2, 1)
    `);
    console.log('✓ Officers seeded');

    console.log('\n✅ Agriculture Database initialization completed successfully!');
    console.log('\nTest Credentials:');
    console.log('  Email: admin@agriculture.gov.in | Password: admin');
    console.log('  Email: hod@agriculture.gov.in | Password: hod');
    console.log('  Email: clerk@agriculture.gov.in | Password: clerk');

  } catch (error) {
    console.error('❌ Error initializing agriculture database:', error.message);
    process.exit(1);
  } finally {
